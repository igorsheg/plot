...
A managed Session survives dashboard disconnection. The terminal dashboard and Fleet Web Console are clients of that managed Session. Application code can instead own a value-only, in-process Session with `createPlot()`.
...
| [Programmatic](programmatic.md) | Run Plot in-process from TypeScript values.                   |
