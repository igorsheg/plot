...
7. **Release shape** — distribute the CLI as Bun single-executable platform packages behind the npm umbrella package `plot-ai`, exposing the binary as `plot`. Keep release machinery in `scripts/release` aligned with `packages/cli/src/main.ts`.
...
```bash
PLOT_VERSION=0.0.0-test bun run release:build
bun run release:smoke
PLOT_CHANNEL=beta bun run release:publish:dry-run
```
