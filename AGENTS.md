...
3. **Plugins are trusted code** — plugins observe the world, reconcile facts, and write normal TypeScript/Effect code to decide what to do next. Core schedules plugin work and tracks running/completed/stalled work; it must not invent a fine-grained capability/grant DSL for every inner tool or command the agent may choose.
