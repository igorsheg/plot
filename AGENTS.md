...
3. **Authority stays in core** — agents/plugins may propose observations, facts, and actions, but core decides what state changes and effects are admitted. Built-ins and user extensions use the same capability declaration, grant, schema validation, idempotency, and dispatch path. Capabilities should be coarse runtime/environment surfaces (for example: start an agent turn in this workspace with this prompt/sandbox), not fine-grained lists of every inner tool or command the agent may choose.
