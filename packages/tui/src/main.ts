...
function updateTable() {
  ...
  const rows = currentState.running.map((r, i) => {
    ...
    const id = isSelected ? fg(C.cyan, bold(r.issueIdentifier)) : r.issueIdentifier;
    return [
      pad(id, 14),
      pad(r.state, 10),
      ...
      : s.phase === "tool_execution"
        ...
        : s.phase === "compacting"
          ...
          : s.phase === "retrying"
            ...
            : C.muted;
  const toolNames =
    s.activeTools.length > 0 ? s.activeTools.map((at) => at.toolName).join(", ") : "—";
...
export async function runTui(options: { api: RuntimeApi }) {
  ...
  const done = new Promise<void>((resolve, reject) => {
    ...
    void (async () => {
      ...
      observabilityText = new Text(fg(C.muted, "Waiting for runtime snapshot"), 1);
