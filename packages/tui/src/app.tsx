          ...
          return (
            <box key={entry.issueId} style={{ width: "100%", marginBottom: 0 }}>
              <text fg={selected ? "#22d3ee" : "#e4e4e7"}>
                {selected ? (
                  <strong>{"› "}{entry.issueIdentifier} {entry.state}</strong>
                ) : (
              ...
              return (
                <box key={entry.issueId}>
                  <text fg={selected ? "#22d3ee" : "#fbbf24"}>
                    {selected ? (
                      <strong>{"› "}↻ {entry.identifier} · attempt {entry.attempt}</strong>
                    ) : (
      ...
      {events.length === 0 ? (
        ...
        <scrollbox focused={false} style={{ width: "100%", flexGrow: 1 }}>
          {events.map((event, index) => {
            ...
            return (
              <text key={`${toEpochMs(event.timestamp)}-${index}`} fg={selected ? "#22d3ee" : "#a1a1aa"}>
                {selected ? "› " : "  "}
...
function DetailPane({ event }: { event: AgentRuntimeEvent | null }) {
  ...
  if (event.usage) {
    fields.push({ key: "in", value: formatTokens(event.usage.inputTokens), color: "#34d399" })
    fields.push({ key: "out", value: formatTokens(event.usage.outputTokens), color: "#34d399" })
    fields.push({ key: "total", value: formatTokens(event.usage.totalTokens), color: "#34d399" })
  }
...
function OpsPanel({ snapshot }: { snapshot: RuntimeSnapshot | null }) {
  ...
  return (
    <box title="ops" border borderStyle="single" style={{ width: 34, height: "100%" }}>
      ...
      <text fg="#71717a">
        depth {o.commandQueueDepth} · peak {o.commandQueuePeak} · pressure {o.commandQueuePressureCount}
      </text>
      <text> </text>
      <text fg="#e4e4e7">
...
function Footer({ opsVisible }: { opsVisible: boolean }) {
  ...
  return (
    <box style={{ height: 1, width: "100%" }}>
      <text fg="#71717a">j/k move │ h/l focus │ tab switch │ o ops {opsLabel} │ r refresh │ q quit</text>
    </box>
  )
}
...
export function App({ api }: { api: RuntimeApi }) {
  ...
  const issueDetailIdRef = useRef<string | null>(null)
  ...
  // auto-focus
  useEffect(() => {
    if (railIdentifiers.length > 0 && (!focusedIdentifier || !railIdentifiers.includes(focusedIdentifier))) {
      setFocusedIdentifier(railIdentifiers[0] ?? null)
      setEventLog(null)
      setIssueDetail(null)
      setSelectedEventIndex(0)
      eventLogIdRef.current = null
      issueDetailIdRef.current = null
    }
    ...
    const disconnectEvents = api.connectEvents((event) => {
      setEventLog((prev) => {
        ...
        if (
          ...
          return { ...prev, events: [...prev.events.slice(0, -1), merged] } as IssueEventLog
        }
        return { ...prev, events: [...prev.events, event] } as IssueEventLog
      })
    })
  ...
  const events = eventLog?.events ?? issueDetail?.eventTail ?? []
  useEffect(() => {
    if (events.length > 0) {
  ...
  return (
    <box flexDirection="column" style={{ width: "100%", height: "100%" }}>
      ...
      <box flexDirection="row" style={{ flexGrow: 1, width: "100%", gap: 1 }}>
        <WorkRail snapshot={snapshot} focusedIdentifier={focusedIdentifier} isActive={focusPane === "issues"} />
