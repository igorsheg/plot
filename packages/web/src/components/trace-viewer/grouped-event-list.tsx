...
function formatRelative(base: DateTime.Utc, event: DateTime.Utc): string {
	const diff =
		Number(DateTime.toEpochMillis(event)) -
		Number(DateTime.toEpochMillis(base));
...
function formatTimestamp(dt: DateTime.Utc): string {
		...
		<div>
			<div className="trace-turn-header flex items-center gap-2 border-b border-border/50 bg-muted/30 px-3 py-2 type-body">
				...
				<span className="flex shrink-0 items-center gap-2 type-meta tabular-nums">
					{turn.tokenUsage && (
						<span>{turn.tokenUsage.totalTokens.toLocaleString()} tok</span>
					)}
			...
			{items.map((item, i) => {
				...
				if (item.kind === "tool") {
					...
					return (
						...
						>
							<span className="w-4 shrink-0 type-meta">
								{connector}
							</span>
							<span className={cn("w-[72px] shrink-0 truncate", color)}>
								{tc.toolName}
							</span>
							<span className="min-w-0 flex-1 truncate text-foreground">
				...
				return (
					<div
						key={`notif-${i}`}
						className="flex items-center gap-2 px-3 py-1 pl-6 type-body"
					>
						<span className="w-4 shrink-0 type-meta">
							{connector}
						</span>
...
function UngroupedEvents({ section }: { section: UngroupedSection }) {
	const label =
		section.kind === "preamble" ? "agent lifecycle" : "agent shutdown";
	...
	return (
		<div>
			...
			{section.events.map((ev, i) => (
				<div
					key={`${section.kind}-${i}`}
					className="flex items-center gap-2 px-3 py-1 type-meta"
				>
...
export function GroupedEventList() {
	...
	const groups = useMemo(
		() => groupEventsIntoTurns(state.events),
		[state.events],
	);
