...
function findActiveToolCall(
	...
	if (toolCallId) {
		return turn.toolCalls.find(
			(tc) => tc.toolCallId === toolCallId && tc.isActive,
		);
	}
	...
	for (const ev of events) {
		...
		switch (ev.event) {
			...
			case "turn_end": {
				if (currentTurn) {
					...
					currentTurn.durationMs =
						epochMs(ev.timestamp) - epochMs(currentTurn.startedAt);
			...
			case "auto_retry_start": {
				if (currentTurn) {
					const name =
						ev.event === "auto_compaction_start" ? "compaction" : "retry";
			...
			case "auto_retry_end": {
				if (currentTurn) {
					const name =
						ev.event === "auto_compaction_end" ? "compaction" : "retry";
					const tc = currentTurn.toolCalls.find(
						(t) => t.toolName === name && t.isActive,
					);
					if (tc) {
						tc.events.push(ev);
						tc.endedAt = ev.timestamp;
	...
	if (lastTurnEndIndex >= 0 && lastTurnEndIndex < events.length - 1) {
		...
		for (const ev of events.slice(lastTurnEndIndex + 1)) {
			...
			const alreadyGrouped = groups.some(
				(g) => g.kind === "turn" && g.events.includes(ev),
			);
			if (!alreadyGrouped) {
		...
		if (group.kind === "turn") {
			const turnMatchesQuery =
				...
				group.toolCalls.some(
					(tc) =>
						matchesQuery(tc.toolName, query) || matchesQuery(tc.summary, query),
				) ||
			...
			if (typeFilter.size === 0) {
				...
				const filteredToolCalls = group.toolCalls.filter((tc) =>
					typeFilter.has(tc.toolName),
				);
