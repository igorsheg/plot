...
export function makeAppLayer(resolved: ResolvedConfig) {
  return Layer.mergeAll(makeTrackerLayer(resolved), PiAgentLive, BunContext.layer);
}
...
export function makeObservabilityLayer(resolved: ResolvedConfig) {
  return ObservabilityApi.Default.pipe(Layer.provide(makeOrchestratorLayer(resolved)));
}
