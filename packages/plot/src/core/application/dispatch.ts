...
export interface DispatchDeps {
  ...
  readonly retryTimerFibersRef: Ref.Ref<Map<string, Fiber.RuntimeFiber<void, never>>>;
...
export function makeDispatchRuntime(deps: DispatchDeps) {
  ...
  const replaceRetryTimerFiber = (issueId: string, fiber: Fiber.RuntimeFiber<void, never>) =>
    Ref.modify(deps.retryTimerFibersRef, (timers) => {
      const next = new Map(timers);
      ...
      const shouldContinue = () =>
        deps.tracker.fetchIssueStatesByIds([issue.id]).pipe(
          ...
          Effect.map((result) => {
            ...
            return isDispatchable(entry.state, config) && !isTerminal(entry.state, config);
          }),
        );
