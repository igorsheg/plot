...
plot always loads its bundled runtime skills. these are packaged with the orchestrator and available to every agent run without any setup.

when the target workspace contains a `.agent/skills` or `.claude/skills` directory, plot loads those repo-local skills as well. this lets each repository ship its own skills alongside the code, available to any agent that works in that repo.

user-global skills (e.g. those installed in `~/.pi/skills` or similar user-level directories) are not loaded. the runtime contract only includes bundled skills and repo-local skills — nothing from the user's home directory leaks into orchestrated runs.
